"""
Download HDFC Mid Cap Fund portfolio files using Playwright.

HDFC page has year/month dropdowns to filter data.
This script navigates through each month to extract Mid Cap Fund URLs.
...
def parse_date_from_filename(filename: str) -> str | None:
    """Extract date (YYYY-MM-DD) from filename like 'Monthly HDFC Mid Cap Fund - 31 December 2025.xlsx'."""
    match = re.search(
...
def extract_midcap_url(page) -> str | None:
    """Extract Mid Cap Fund URL from current page view.

    Fund name variations:
    - Current: "HDFC Mid Cap Fund"
    - Old: "HDFC Mid-Cap Opportunities Fund"
    """
    links = page.evaluate('''() => {
        const links = [];
        document.querySelectorAll('a').forEach(a => {
            ...
            if (href && href.includes('.xlsx')) {
                ...
                // Match "Mid Cap Fund" or "Mid-Cap Opportunities Fund"
                // but exclude NIFTY and "Large and Mid"
                const hasTarget = (
                    decoded.includes('Mid Cap Fund') ||
                    decoded.includes('Mid-Cap Opportunities Fund')
                );
                const excluded = (
                    decoded.toUpperCase().includes('NIFTY') ||
                    decoded.includes('Large and Mid') ||
                    decoded.includes('Large & Mid')
                );
...
def download_files():
    """Download Mid Cap Fund files for last 13 months."""
    ...
    with sync_playwright() as p:
        ...
        for year, month, month_name, fiscal_year in target_months:
            ...
            # Step 3: Extract Mid Cap Fund URL
            midcap_url = extract_midcap_url(page)

            if not midcap_url:
                print(f"  -> [FAIL] No Mid Cap Fund file found")
                results.append({"month": f"{month_name} {year}", "success": False})
                continue

            filename = unquote(midcap_url.split('/')[-1])
            ...
            output_filename = f"hdfc_midcap_{file_date or f'{year}-{month:02d}'}.xlsx"
