"""
Download HDFC Large Cap Fund portfolio files using Playwright.

HDFC page has year/month dropdowns to filter data.
This script navigates through each month to extract Large Cap Fund URLs.
...
def parse_date_from_filename(filename: str) -> str | None:
    """Extract date (YYYY-MM-DD) from filename like 'Monthly HDFC Large Cap Fund - 31 December 2025.xlsx'."""
    match = re.search(
...
def extract_largecap_url(page) -> str | None:
    """Extract Large Cap Fund URL from current page view.

    Fund name: "HDFC Large Cap Fund" or "HDFC Top 200 Fund" (old name)
    """
    links = page.evaluate('''() => {
        const links = [];
        document.querySelectorAll('a').forEach(a => {
            ...
            if (href && href.includes('.xlsx')) {
                ...
                // Match "Large Cap Fund" or "Top 200 Fund" (old name)
                const hasTarget = (
                    decoded.includes('Large Cap Fund') ||
                    decoded.includes('Top 200 Fund')
                );
                const excluded = (
...
def download_files():
    ...
    with sync_playwright() as p:
        ...
        for year, month, month_name, fiscal_year in target_months:
            ...
            # Step 3: Extract Large Cap Fund URL
            largecap_url = extract_largecap_url(page)

            if not largecap_url:
                print(f"  -> [FAIL] No Large Cap Fund file found")
                results.append({"month": f"{month_name} {year}", "success": False})
                continue

            filename = unquote(largecap_url.split('/')[-1])
            ...
            output_filename = f"hdfc_largecap_{file_date or f'{year}-{month:02d}'}.xlsx"
