"""
Download HDFC Flexi Cap Fund portfolio files using Playwright.

HDFC page has year/month dropdowns to filter data.
This script navigates through each month to extract Flexi Cap Fund URLs.
...
def parse_date_from_filename(filename: str) -> str | None:
    """Extract date (YYYY-MM-DD) from filename like 'Monthly HDFC Flexi Cap Fund - 31 December 2025.xlsx'."""
    match = re.search(
...
def extract_flexicap_url(page) -> str | None:
    """Extract Flexi Cap Fund URL from current page view.
    ...
    links = page.evaluate('''() => {
        ...
        document.querySelectorAll('a').forEach(a => {
            ...
            if (href && href.includes('.xlsx')) {
                ...
                // Match "Flexi Cap Fund" but exclude Large and Mid variants
                const hasTarget = decoded.includes('Flexi Cap Fund');
                const excluded = (
...
def download_files():
    """Download Flexi Cap Fund files for last 13 months."""
    ...
    with sync_playwright() as p:
        ...
        for year, month, month_name, fiscal_year in target_months:
            ...
            # Step 3: Extract Flexi Cap Fund URL
            flexicap_url = extract_flexicap_url(page)

            if not flexicap_url:
                print(f"  -> [FAIL] No Flexi Cap Fund file found")
                results.append({"month": f"{month_name} {year}", "success": False})
                continue

            filename = unquote(flexicap_url.split('/')[-1])
            ...
            output_filename = f"hdfc_flexicap_{file_date or f'{year}-{month:02d}'}.xlsx"
